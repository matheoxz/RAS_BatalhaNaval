...
class Tela(Tk):
    ...
    def checaTiro(self, x, y, jogador, modo='Sandbox', ganhou=False, incrementa=True):
        if(jogador == 'Player'):
            ...
            if incrementa:
                ...
                self.tiros_dados_player_lbl.configure(
                    text=str(self.tiros_dados_player))

            tiro, afundados = self.tabuleiro.checaTiro(x, y, mat)
            print('Player', afundados)
            if(tiro == 0):
                self.matriz_inimiga[x][y].configure(
                    bg="#5151B8", state=DISABLED)
                if incrementa:
                    self.tiros_errados_player += 1
                    self.tiros_errados_player_lbl.configure(
                        text=str(self.tiros_errados_player))

            elif(tiro[0] == 2):
                self.matriz_inimiga[x][y].configure(
                    bg="#2A9D8F", text=str(tiro[1]+1), fg='black', state=DISABLED)
                if incrementa:
                    self.tiros_acertados_player += 1
                    self.tiros_acertados_player_lbl.configure(
                        text=str(self.tiros_acertados_player))
                    self.destroyer_acertados_lbl.configure(
                        text=str(afundados[3])+'/'+str(self.navios[3]))

            elif(tiro[0] == 3):
                self.matriz_inimiga[x][y].configure(
                    bg="#E9C46A", text=str(tiro[1]+1), fg='black', state=DISABLED)
                if incrementa:
                    self.tiros_acertados_player += 1
                    self.tiros_acertados_player_lbl.configure(
                        text=str(self.tiros_acertados_player))
                    self.submarine_acertados_lbl.configure(
                        text=str(afundados[2])+'/'+str(self.navios[2]))

            elif(tiro[0] == 4):
                self.matriz_inimiga[x][y].configure(
                    bg="#F4A261", text=str(tiro[1]+1), fg='black', state=DISABLED)
                if incrementa:
                    self.tiros_acertados_player += 1
                    self.tiros_acertados_player_lbl.configure(
                        text=str(self.tiros_acertados_player))
                    self.battleship_acertados_lbl.configure(
                        text=str(afundados[1])+'/'+str(self.navios[1]))

            elif(tiro[0] == 5):
                self.matriz_inimiga[x][y].configure(
                    bg="#E76F51", text=str(tiro[1]+1), fg='black', state=DISABLED)
                if incrementa:
                    self.tiros_acertados_player += 1
                    self.tiros_acertados_player_lbl.configure(
                        text=str(self.tiros_acertados_player))
                    self.carrier_acertados_lbl.configure(
                        text=str(afundados[0])+'/'+str(self.navios[0]))
            ...
            if incrementa:
                ...
                self.tiros_dados_bot_lbl.configure(
                    text=str(self.tiros_dados_bot))
            print('Bot', x, y)
            ...
            if(tiro == 0):
                ...
                if incrementa:
                    ...
                    self.tiros_errados_bot_lbl.configure(
                        text=str(self.tiros_errados_bot))

            elif(tiro[0] == 2):
                self.matriz_amiga[x][y].configure(
                    bg="#2A9D8F", text=str(tiro[1]+1), fg='black', state=DISABLED)
                if incrementa:
                    self.tiros_acertados_bot += 1
                    self.tiros_acertados_bot_lbl.configure(
                        text=str(self.tiros_acertados_bot))
                    self.destroyer_acertados_bot_lbl.configure(
                        text=str(afundados[3])+'/'+str(self.navios[3]))
                ...
                if incrementa:
                    ...
                    self.tiros_acertados_bot_lbl.configure(
                        text=str(self.tiros_acertados_bot))
                    self.submarine_acertados_bot_lbl.configure(
                        text=str(afundados[2])+'/'+str(self.navios[2]))
                ...
                if incrementa:
                    ...
                    self.tiros_acertados_bot_lbl.configure(
                        text=str(self.tiros_acertados_bot))
                    self.battleship_acertados_bot_lbl.configure(
                        text=str(afundados[1])+'/'+str(self.navios[1]))
                ...
                if incrementa:
                    ...
                    self.tiros_acertados_bot_lbl.configure(
                        text=str(self.tiros_acertados_bot))
                    self.carrier_acertados_bot_lbl.configure(
                        text=str(afundados[0])+'/'+str(self.navios[0]))
    ...
    def criaMatrizDeBotao(self, frame, matriz_botoes, jogador, modo='Sandbox'):
        ...
        if jogador == 'Player':
            for i, j in Matriz.geraMatriz(Matriz, self.n):
                checaTiro_xy = partial(
                    self.checaTiro, i, j, 'Player', modo=modo)
                matriz_botoes[i][j] = Button(
                    frame, text='~', height=x, width=y, bg="#BFBFF2", command=checaTiro_xy)
                matriz_botoes[i][j].grid(row=i+1, column=j+1)

        else:
            for i, j in Matriz.geraMatriz(Matriz, self.n):
                matriz_botoes[i][j] = Button(
                    frame, text='~', height=x, width=y, bg="#BFBFF2", state=DISABLED)
    ...
    def getNumeros(self):
        try:
            ...
            self.n = self.tabuleiro.getN()
            assert self.n_user >= self.n, 'O tamanho da matriz deve ser no mínimo {}'.format(
                self.n)
    ...
    def criaTelaConfigurações(self):
        ...
        # insere labels
        Label(frame, text='Carrier (5 espaços): ').grid(
            row=1, column=0, sticky=W)
        Label(frame, text='Battleship (4 espaços): ').grid(
            row=2, column=0, sticky=W)
        Label(frame, text='Submarine (3 espaços): ').grid(
            row=3, column=0, sticky=W)
        Label(frame, text='Destroyer (2 espaços): ').grid(
            row=4, column=0, sticky=W)
        Label(frame, text='Percentual de água: ').grid(
            row=5, column=0, sticky=W)
        Label(frame, text='Tamanho do tabuleiro').grid(
    ...
    def Sandbox(self):
        ...
        self.matriz_inimiga = [
            [None for i in range(self.n)] for i in range(self.n)]
        ...
        Label(frame_pontuacao, text='Tiros dados: ').grid(
            row=0, column=0, sticky=W)
        self.tiros_dados_player = 0
        self.tiros_dados_player_lbl = Label(
            frame_pontuacao, text=str(self.tiros_dados_player))
        self.tiros_dados_player_lbl.grid(row=0, column=1, sticky=E)
        # label de tiros acertados
        Label(frame_pontuacao, text='Tiros acertados: ').grid(
            row=0, column=2, sticky=W)
        self.tiros_acertados_player = 0
        self.tiros_acertados_player_lbl = Label(
            frame_pontuacao, text=str(self.tiros_acertados_player))
        self.tiros_acertados_player_lbl.grid(row=0, column=3, sticky=E)
        # label de tiros errados
        Label(frame_pontuacao, text='Tiros errados: ').grid(
            row=0, column=4, sticky=W)
        self.tiros_errados_player = 0
        self.tiros_errados_player_lbl = Label(
            frame_pontuacao, text=str(self.tiros_errados_player))
        self.tiros_errados_player_lbl.grid(row=0, column=5, sticky=E)
        # labels navios
        # carrier
        Label(frame_pontuacao, text='Carrier: ').grid(
            row=1, column=0, sticky=W)
        self.carrier_acertados_lbl = Label(
            frame_pontuacao, text='0/'+str(self.navios[0]))
        self.carrier_acertados_lbl.grid(row=1, column=1, sticky=E)
        # battleship
        Label(frame_pontuacao, text='Battleship: ').grid(
            row=2, column=0, sticky=W)
        self.battleship_acertados_lbl = Label(
            frame_pontuacao, text='0/'+str(self.navios[1]))
        self.battleship_acertados_lbl.grid(row=2, column=1, sticky=E)
        # submarine
        Label(frame_pontuacao, text='Submarine: ').grid(
            row=3, column=0, sticky=W)
        self.submarine_acertados_lbl = Label(
            frame_pontuacao, text='0/'+str(self.navios[2]))
        self.submarine_acertados_lbl.grid(row=3, column=1, sticky=E)
        # destroyer
        Label(frame_pontuacao, text='Destroyer: ').grid(
            row=4, column=0, sticky=W)
        self.destroyer_acertados_lbl = Label(
            frame_pontuacao, text='0/'+str(self.navios[3]))
    ...
    def PvE(self):
        ...
        self.matriz_inimiga = [
            [None for i in range(self.n)] for i in range(self.n)]
        #self.matriz_inimiga = self.tabuleiro.alocaNavios()
        self.criaMatrizDeBotao(
            frame_tabuleiro, self.matriz_inimiga, 'Player', modo='PvE')
        ...
        # cria o tabuleiro
        frame_tabuleiro_amigo = LabelFrame(
            self.sandbox_window, text="Campo Amigo")
        # coloca labels de coordenadas
        for i in range(self.n):
        ...
        self.criaMatrizDeBotao(frame_tabuleiro_amigo,
                               self.matriz_amiga, 'Bot', modo='PvE')
        frame_tabuleiro_amigo.grid(row=0, column=1, padx=10)
        # cria labels de pontuação
        frame_pontuacao_inimigo = LabelFrame(
            self.sandbox_window, text='Pontuação do Inimigo')
        # label de tiros dados
        Label(frame_pontuacao_inimigo, text='Tiros dados: ').grid(
            row=0, column=0, sticky=W)
        self.tiros_dados_bot = 0
        self.tiros_dados_bot_lbl = Label(
            frame_pontuacao_inimigo, text=str(self.tiros_dados_bot))
        self.tiros_dados_bot_lbl.grid(row=0, column=1, sticky=E)
        # label de tiros acertados
        Label(frame_pontuacao_inimigo, text='Tiros acertados: ').grid(
            row=0, column=2, sticky=W)
        self.tiros_acertados_bot = 0
        self.tiros_acertados_bot_lbl = Label(
            frame_pontuacao_inimigo, text=str(self.tiros_acertados_bot))
        self.tiros_acertados_bot_lbl.grid(row=0, column=3, sticky=E)
        # label de tiros errados
        Label(frame_pontuacao_inimigo, text='Tiros errados: ').grid(
            row=0, column=4, sticky=W)
        self.tiros_errados_bot = 0
        self.tiros_errados_bot_lbl = Label(
            frame_pontuacao_inimigo, text=str(self.tiros_errados_bot))
        self.tiros_errados_bot_lbl.grid(row=0, column=5, sticky=E)
        # labels navios
        # carrier
        Label(frame_pontuacao_inimigo, text='Carrier: ').grid(
            row=1, column=0, sticky=W)
        self.carrier_acertados_bot_lbl = Label(
            frame_pontuacao_inimigo, text='0/'+str(self.navios[0]))
        self.carrier_acertados_bot_lbl.grid(row=1, column=1, sticky=E)
        # battleship
        Label(frame_pontuacao_inimigo, text='Battleship: ').grid(
            row=2, column=0, sticky=W)
        self.battleship_acertados_bot_lbl = Label(
            frame_pontuacao_inimigo, text='0/'+str(self.navios[1]))
        self.battleship_acertados_bot_lbl.grid(row=2, column=1, sticky=E)
        # submarine
        Label(frame_pontuacao_inimigo, text='Submarine: ').grid(
            row=3, column=0, sticky=W)
        self.submarine_acertados_bot_lbl = Label(
            frame_pontuacao_inimigo, text='0/'+str(self.navios[2]))
        self.submarine_acertados_bot_lbl.grid(row=3, column=1, sticky=E)
        # destroyer
        Label(frame_pontuacao_inimigo, text='Destroyer: ').grid(
            row=4, column=0, sticky=W)
        self.destroyer_acertados_bot_lbl = Label(
            frame_pontuacao_inimigo, text='0/'+str(self.navios[3]))
    ...
    def criaTelaMenu(self):
        ...
        config_button = Button(frame, text="Configurações",
                               command=self.criaTelaConfigurações)
        config_button.pack(padx=5, pady=5, fill=BOTH)
        # insere botao de sair
        exit_button = Button(frame, text="Sair", command=self.quit).pack(
