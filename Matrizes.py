...
class Matriz:
    ...
    def defineTamanhoMatriz(self):
        espacos = (self.navios[0]*5+self.navios[1]*4 +
                   self.navios[2]*3+self.navios[3]*2)/(1-self.pma)
    ...
    def __init__(self, pma, navios, n=None):
        ...
        print('Coordenadas bot: ', self.coordenadas)
    ...
    def alocaNavios(self):  # gustavo e marcos
        ...
        # submarine = 3 espaços
        # destroyer = 2 espaços
        # espaço que cada navio ocupa idx 0:carrier 1:battleship 2:submarine 3:destroyer
        espaco_navio = (5, 4, 3, 2)
        n = self.n
        ...
        while(qtde_navios > 0):
            ...
            # coordenada inicial e final inicializadas
            c_i = [randint(0, n-1), randint(0, n-1)]
            c_f = c_i.copy()
            ...
            while(verifica != 2 and espacos_alocados < espaco_navio[i]):
                if(matriz_navios[c_f[0]][c_f[1]] == 0 and c_f[pos] != -1):
                    espacos_alocados += 1
                    c_f[pos] = c_f[pos] + \
                        step if((c_f[pos]+step) <
                                n and (c_f[pos]+step) >= 0) else -1
                else:
                    verifica += 1
                    espacos_alocados = 0
            ...
            if(verifica < 2):  # Caso verifica <2 há a possiblidade de alocar o navio no sentido determinado pelo step
                for x in range(espaco_navio[i]):
                    # alocando o navio
                    matriz_navios[c_i[0]+step *
                                  abs(pos-1)*x][c_i[1]+step*pos*x] = (tipo, x, qtde_tipo)
                qtde_navios -= 1
                qtde_tipo -= 1
        # retorna uma matriz com os navios alocados para poder ser atribuida a matriz do bot/player
    ...
    def geraTiro(self, matriz_bot):
        ...
        if(self.bot_navio == []):
            ...
            # [Step_linha Step_coluna Tipo Identificador Lados_Verificados Espaços_encontrados_daquele_navio Problema]
            step = self.bot_navio.pop()
            if(step[5] == 0):
                step[4] += 1
                while(1):
                    if(step[4] == 1):
                    ...
                    if(step[4] == 4):
                        step[0] = 0
                        step[1] = -1
                    c_test = [self.bot_navio[step[5]][0]+step[0],
                              self.bot_navio[step[5]][1]+step[1]]
            ...
            if(not step[6]):           # O step é sempre o último elemento
                c = [self.bot_navio[step[5]][0]+step[0],
                     self.bot_navio[step[5]][1]+step[1]]
            else:
                c = [self.bot_navio[0][0]+step[0],
                     self.bot_navio[0][1]+step[1]]
        if(matriz_bot[c[0]][c[1]] != 0):
            if(step[5] == -1):  # verifica se o primeiro elemento de um tipo
                step = [0, 0, matriz_bot[c[0]][c[1]][0], matriz_bot[c[0]]
                        [c[1]][2], 0, step[5]+1, 0]  # monta o próximo passo
                ...
                # Outros elementos que contenham navios diferentes
                self.bot_navio_extra.append(c)
                self.bot_navio.append(step)
        ...
        if(step[5] == (step[2]-1)):
            ...
            if(self.bot_navio_extra != []):
                ...
                step = [0, 0, matriz_bot[self.bot_navio[0][0]][self.bot_navio[0][1]][0],
                        matriz_bot[self.bot_navio[0][0]][self.bot_navio[0][1]][2], 0, 0, 0]
                self.bot_navio.append(step)
        self.coordenadas.append(c)
        return c
